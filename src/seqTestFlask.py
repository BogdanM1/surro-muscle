...
#
f=open('result.csv','w')
f.write('time, id, sig_pred, dsig_pred, sig, dsig\n')
test_data = {}
test_data_counter = 0
#
simulation_data = {}
...
def start():
  global simulation_data, test_data, test_data_counter
  params = request.form.to_dict()
  ...
  test_data = pd.read_csv("../data/tests/test" + str(simulation_id) + ".csv")
  test_data_counter = 0
  return "OK"
...
def prediction():
  global models, simulation_data, test_data, test_data_counter
  ...
  for i in range(count):
    result += str(test_data_counter)+ ',' + str(i) + ',' + str(sigma_predicted[i]) + ',' + str(dsigma_predicted[i]) + ','  
    result += str(test_data.iloc[test_data_counter+i,10]) + ',' + str(test_data.iloc[test_data_counter+i,12])+"\n"
  f.write(result)  
  f.flush()
  result = ""
  count = len(sigma_predicted)
